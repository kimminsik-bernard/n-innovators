...
if ( $wpex_query->have_posts() ) :
	...
	if ( $filter_taxonomy ) {
		...
		if ( $filter_terms ) {
			...
			if ( isset( $_GET[$filter_url_param] ) && $filter_active_category = esc_html( $_GET[$filter_url_param] ) ) {
				if ( ! is_numeric( $filter_active_category ) ) {
					$get_term = get_term_by( 'name', $filter_active_category, $filter_taxonomy );
					if ( $get_term ) {
						$atts['filter_active_category'] = $get_term->term_id;
					}
				} else {
					$atts['filter_active_category'] = $filter_active_category;
				}
			}
							...
							elseif ( has_post_thumbnail( $latts['post_id'] ) ) :
								...
								$output .= '<div class="'. $media_classes .'">';
									...
									$output .= wpex_get_post_thumbnail( array(
										...
										'apply_filters' => 'vcex_grid_thumbnail_args',
										'filter_arg1'   => $latts,
									) );
