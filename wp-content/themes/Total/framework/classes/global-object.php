...
class WPEX_Global_Theme_Object {
	...
	private function main_layout() {
		...
		// Check meta
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_main_layout', true ) ) {
			$layout = $meta;
		}
	...
	private function post_layout() {
		...
		// Define variables
		$class = 'right-sidebar';
		$meta  = $this->post_id ? get_post_meta( $this->post_id, 'wpex_post_layout', true ) : '';
	...
	private function has_header() {
		...
		// Check meta
		$meta = $this->post_id ? get_post_meta( $this->post_id, 'wpex_disable_header', true ) : '';
	...
	private function header_overlay_logo() {
		...
		$logo = get_post_meta( $this->post_id, 'wpex_overlay_header_logo', true );
	...
	private function header_style() {
		...
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_header_style', true ) ) {
			$style = $meta;
		}
	...
	private function fixed_header_style() {
		...
		if ( 'disabled' == $style
			&& $this->post_id
			&& 'enable' == get_post_meta( $this->post_id, 'wpex_sticky_header', true ) ) {
			$style = 'standard';
		}
	...
	private function has_fixed_header() {
		$return = false;
		// Disable for header builder and in VC Live editor
		if ( $this->header_builder || $this->vc_is_inline ) {
			return false;
		}
		if ( 'disabled' != $this->fixed_header_style
			&& ( 'one' == $this->header_style || 'five' == $this->header_style )
		) {
			$return = true;
		}
		if ( $this->post_id && 'disable' == get_post_meta( $this->post_id, 'wpex_sticky_header', true ) ) {
			$return = false;
		}
	...
	private function post_slider_position() {
		...
		if ( $this->has_post_slider ) {
			...
			if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_post_slider_shortcode_position', true ) ) {
				$position = $meta;
			}
	...
	private function page_header_style() {
		...
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_post_title_style', true ) ) {
			$style = $meta;
		}
	...
	private function has_page_header() {
		...
		// Return false if page header is disabled via custom field
		if ( $this->post_id ) {
	...
	private function has_page_header_title() {
		...
		if ( $this->post_id && 'on' == get_post_meta( $this->post_id, 'wpex_disable_title', true ) ) {
			return false;
		}
	...
	private function get_page_subheading() {
		...
		// Posts & Pages
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_post_subheading', true ) ) {
			$subheading = $meta;
		}
	...
	private function has_breadcrumbs() {
		...
		// Check page settings
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_disable_breadcrumbs', true ) ) {
			if ( 'on' == $meta ) {
				$return = false;
			} elseif ( 'enable' == $meta ) {
				$return = true;
	...
	private function has_footer() {
		...
		// Check page settings
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_disable_footer', true ) ) {
			if ( 'on' == $meta ) {
				$return = false;
			} elseif ( 'enable' == $meta ) {
				$return = true;
	...
	private function has_footer_widgets() {
		...
		// Check page settings
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_disable_footer_widgets', true ) ) {
			if ( 'on' == $meta ) {
				$return = false;
			} elseif ( 'enable' == $meta ) {
				$return = true;
	...
	private function has_footer_reveal() {
		...
		// Check page settings
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_footer_reveal', true ) ) {
			if ( 'on' == $meta ) {
				$return = true;
			} elseif ( 'off' == $meta ) {
				$return = false;
	...
	private function has_footer_callout() {
		...
		// Check page settings
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_disable_footer_callout', true ) ) {
			if ( 'on' == $meta ) {
				$return = false;
			} elseif ( 'enable' == $meta ) {
				$return = true;
	...
	private function has_social_share() {
		...
		if ( $this->post_id && $meta = get_post_meta( $this->post_id, 'wpex_disable_social', true ) ) {
