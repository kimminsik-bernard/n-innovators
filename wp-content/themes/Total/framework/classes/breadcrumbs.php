	...
	class WPEX_Breadcrumbs {
		...
		private function generate_crumbs() {
			...
			elseif ( is_archive() ) {
				...
				elseif ( is_date() ) {
					...
					if ( is_day() ) {
						...
						$link  = get_month_link( get_the_time( 'Y' ) );
			...
			if ( $trail && is_array( $trail ) ) {
				...
				// Add to trail
				if ( isset( $trail['trail_end'] ) ) {
					$trail['trail_end'] = '<span itemprop="title" class="trail-end">'. esc_html( $trail['trail_end'] ) .'</span>';
				}
				...
				$header_style = wpex_global_obj( 'page_header_style' );
