	...
	class WPEX_Gallery_Metabox {
		...
		public function __construct() {
			...
			foreach( $this->post_types as $key => $val ) {
				add_action( 'add_meta_boxes_'. $val, array( 'WPEX_Gallery_Metabox', 'add_meta' ), 20 );
			}
		...
		public static function add_meta( $post ) {
			add_meta_box(
				...
				array( 'WPEX_Gallery_Metabox', 'render' ),
				$post->post_type,
				'normal',
				'high'
			);
		...
		public function load_scripts( $hook ) {
			...
			// CSS
			wp_enqueue_style( 'total-gmb-css', wpex_asset_url( 'lib/gallery-metabox/gmb.css' ) );
			...
			wp_enqueue_script(
				'wpex-gmb-js',
				wpex_asset_url( 'lib/gallery-metabox/gmb.js' ),
