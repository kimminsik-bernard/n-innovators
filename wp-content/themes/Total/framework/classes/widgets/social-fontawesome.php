					...
					foreach( $social_services as $key => $val ) :
						...
						if ( $link ) {
							...
							$output .= '<li>';
								...
								$output .= '<a href="'. esc_url( $link ) .'" title="'. esc_attr( $name ) .'" class="wpex-'. esc_attr( $key ) .' '. wpex_get_social_button_class( $style ) .'"'. $add_style . $target . $nofollow .'>';

										$output .= '<span class="fa fa-'. esc_attr( $icon ) .'"></span>';
